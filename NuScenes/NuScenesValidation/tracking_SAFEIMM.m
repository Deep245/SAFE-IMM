...
classdef tracking_SAFEIMM < matlabshared.tracking.internal.AbstractTrackingFilter ...
    ...
    %==================== BESTCHOICE-IMM (existing heuristics) =============
    properties (Access = protected)
        % Winner/state memory
        ...
        % If true we *try* WTA; in this novel version, a gate decides per-step.
    ...
    properties (Access = protected)
        ...
        pEPS_TrPRelTol  single = single(0.05)   % 5% relative trace tolerance
    ...
    properties (Access = protected)
        ...
        pFastMode       = true;
        pUseEntropyCtrl = true;          % we use a cheap sharpness check
    ...
    properties (Access = protected)
        ...
        pRobustKind      = uint8(1);      % default Student-t
        pNu              = single(5.0);   % Student-t degrees of freedom
        pNu_Jam          = single(2.0);   % stronger heavy-tail under jam
        pHuberK          = single(3.0);   % Huber threshold (in sqrt(NIS))
        pHuberK_Jam      = single(1.5);   % tighter under jam
    ...
    % properties (Access = protected)
    %     % ε-gate cache for Cholesky of Pbar (winner-space)
    %     pEPS_LastWinner   = uint32(0);
    %     pEPS_R_chol       % allocated lazily as n-by-n
    %     pEPS_trPbar       = single(0);
    %     pEPS_CacheValid   = false;
    %     pEPS_TrPRelTol    = single(1e-3);  % relative trace-change threshold
    % end
    ...
    methods
        function IMM = tracking_SAFEIMM(varargin)
            ...
            [smoothArgs, idx1, idx2] = tracking_SAFEIMM.parseSmoothingNVPairs(varargin{:});
            ...
            if isempty(modelNames)
                ...
                validateattributes(modelNames, {'cell','string'}, {'numel', numel(trackingFilters)}, 'tracking_SAFEIMM', 'ModelNames');
        ...
        function set.State(IMM, value)
            validateattributes(value, {'double','single'}, {'real','finite','nonsparse','vector'}, 'tracking_SAFEIMM', 'State');
            if coder.internal.is_defined(IMM.pTrackingFilters{1}.State)
                m          = numel(IMM.pTrackingFilters{1}.State);
                classToUse = class(IMM.pTrackingFilters{1}.State);
                value      = cast(value, classToUse);
                coder.internal.assert(numel(value) == m, 'fusion:tracking_SAFEIMM:invalidStateDims', 'State', m);
            end
            IMM.pState = value(:);
        end
        ...
        function set.StateCovariance(IMM, value)
            validateattributes(value, {'double','single'}, {'real','finite','nonsparse','2d','nonempty','square'}, 'tracking_SAFEIMM', 'StateCovariance');
            if coder.internal.is_defined(IMM.pTrackingFilters{1}.State)
                m          = numel(IMM.pTrackingFilters{1}.State);
                classToUse = class(IMM.pTrackingFilters{1}.State);
                value      = cast(value, classToUse);
                coder.internal.assert(size(value,1) == m, 'fusion:tracking_SAFEIMM:invalidStateCovDims', 'StateCovariance', m);
            end
        ...
        function set.TrackingFilters(IMM, value)
            validateattributes(value, {'cell'}, {'nonempty'}, 'tracking_SAFEIMM', 'TrackingFilters');
            coder.internal.errorIf(IMM.pNumModels <= 1, 'fusion:tracking_SAFEIMM:invalidMultiModel', 'TrackingFilters');
            ...
            for k = coder.unroll(1:IMM.pNumModels)
                ...
                coder.internal.errorIf(~isMMCompatible(2), 'fusion:tracking_SAFEIMM:invalidFilterType', class(value{k}));
                cond = ~isequal(class(value{1}.State), class(value{k}.State));
                coder.internal.errorIf(cond, 'fusion:tracking_SAFEIMM:invalidStateType', 'TrackingFilters', class(value{k}.State));
        ...
        function set.TransitionProbabilities(IMM, value)
            validateattributes(value, {'double','single'}, {'real','finite','nonsparse','nonempty','nonnegative','<=',1}, 'tracking_SAFEIMM', 'TransitionProbabilities');
            classToUse = class(IMM.pTrackingFilters{1}.State);
            ...
            elseif isvector(value)
                coder.internal.errorIf(~isequal(numel(value), IMM.pNumModels), 'fusion:tracking_SAFEIMM:invalidNumProbVector', 'TransitionProbabilities', IMM.pNumModels);
                ...
                validateattributes(value, {'double','single'}, {'2d','square'}, 'tracking_SAFEIMM', 'TransitionProbabilities');
                coder.internal.errorIf(~isequal(size(value,1), IMM.pNumModels), 'fusion:tracking_SAFEIMM:invalidNumProbMatrix', 'TransitionProbabilities', IMM.pNumModels);
                cond = ~(sum(abs(sum(value,2) - ones(IMM.pNumModels,1,classToUse))) < cast(1e-5, classToUse));
                coder.internal.errorIf(cond, 'fusion:tracking_SAFEIMM:invalidTransProbMatrix', 'TransitionProbabilities');
                transProb = value;
            end
        ...
        function set.ModelConversionFcn(IMM, value)
            validateattributes(value, {'function_handle'}, {'nonempty'}, 'tracking_SAFEIMM', 'ModelconversionFcn');
        ...
        function set.ModelProbabilities(IMM, value)
            validateattributes(value, {'double','single'}, {'real','finite','nonsparse','nonempty','vector','numel',IMM.pNumModels}, 'tracking_SAFEIMM', 'ModelProbabilities');
        ...
        function set.MeasurementNoise(IMM, value)
            validateattributes(value, {'single','double'}, {'real','finite','nonsparse','2d','nonempty','square'}, 'tracking_SAFEIMM', 'MeasurementNoise');
            classToUse = class(IMM.pTrackingFilters{1}.State);
        ...
        function initialize(IMM, state, stateCov, varargin)
            coder.internal.errorIf(numel(IMM) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'initialize');
        ...
        function [x_pred, P_pred] = predict(IMM, varargin)
            coder.internal.errorIf(numel(IMM) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'predict');
        ...
        function [x_corr, P_corr, mdl_probs] = correct(IMM, z, varargin)
            ...
            coder.internal.errorIf(numel(IMM) > 1, ...
                'fusion:tracking_SAFEIMM:NonScalarFilter', ...
                'tracking_SAFEIMM', 'correct');
            ...
            for kMdl = coder.unroll(1:IMM.pNumModels)
                ...
                if doUpdate
                    ...
                    if IMM.pJamActive && isprop(fk, 'MeasurementNoise')
                        ...
                        scaleR  = cast(2.5, 'like', R_saved);            % 2–3× is typical
            ...
            epsOK = (B_eps <= IMM.pEpsSafeWTA);

            doWTA = IMM.pFastMode && IMM.pBC_UseWinnerTakesAll && passSharp && passGLR && epsOK;
        ...
        function d = distance(IMM, zMatrix, measParams)
            ...
            coder.internal.errorIf((numel(IMM) > 1), 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'distance');
        ...
        function newIMM = clone(IMM)
            coder.inline('never');
            coder.internal.errorIf(numel(IMM) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'clone');
        ...
        function [retroState, retroCov, success] = retrodict(obj, dt)
            coder.internal.errorIf(numel(obj) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'retrodict');
        ...
        function [x_retroCorr, P_retroCorr] = retroCorrect(obj, z, varargin)
            coder.internal.errorIf(numel(obj) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'retroCorrect');
        ...
        function tunableProps = tunableProperties(obj)
            ...
            tunableProps = tunableFilterProperties("tracking_SAFEIMM", [s1; s2], f);
        end
    ...
    methods (Access = ...
        ...
        function sync(IMM, IMM2)
            classToExpect = 'tracking_SAFEIMM';
            validateattributes(IMM2, {classToExpect}, {'scalar'}, 'tracking_SAFEIMM');
            for i = coder.unroll(1:IMM.pNumModels)
        ...
        function nullify(IMM)
            coder.internal.errorIf(numel(IMM) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', 'nullify');
            for kMdl = coder.unroll(1:IMM.pNumModels)
    ...
    methods (Access = protected)
        ...
        function validateCorrectJPDAInputs(filter, z, jpda, fcname, varargin)
            coder.internal.errorIf(numel(filter) > 1, 'fusion:tracking_SAFEIMM:NonScalarFilter', 'tracking_SAFEIMM', fcname);
    ...
    methods (Access = private)
        ...
        % Codegen-safe: no cells/try-catch; fixed-size ops; robust Cholesky with ridge.
            ...
            if reuseOK && (rel <= IMM.pEPS_TrPRelTol)
                R = IMM.pEPS_R_chol;  % reuse cached chol(Pbar)
            else
    ...
    methods (Access = protected)
        ...
        function setMaxNumOOSMSteps(IMM, value)
            if value > 0
                ...
                coder.internal.assert(all(isRetrodictionFilter), 'fusion:tracking_SAFEIMM:expectedRetrodictionFilters');
            end
    ...
    methods (Static = true)
        function retIMM = loadobj(sobj)
            if isscalar(sobj)
                retIMM = tracking_SAFEIMM(sobj.TrackingFilters, sobj.ModelConversionFcn, sobj.TransitionProbabilities);
                loadPrivateProtectedProperties(retIMM, sobj);
            else
                retIMM = tracking_SAFEIMM({sobj.TrackingFilters}, sobj(1).ModelConversionFcn, sobj(1).TransitionProbabilities);
    ...
    methods
        function setMeasurementSizes(obj, measurementSize, measurementNoiseSize)
            validateattributes(measurementSize, {'numeric'}, {'real','positive','integer','scalar'}, 'tracking_SAFEIMM');
            validateattributes(measurementNoiseSize, {'numeric'}, {'real','positive','integer','scalar'}, 'tracking_SAFEIMM');
            for i = coder.unroll(1:obj.pNumModels)
    ...
    methods (Access = private)
        function [state, stateCov, trackingFilters, ...
            ...
            coder.internal.errorIf(firstNVIndex > 4, 'fusion:tracking_SAFEIMM:invalidInputsToConstructor', firstNVIndex - 1);
